#ifndef TABLEMGR_TABLEMGR_H
#define TABLEMGR_TABLEMGR_H
...
public:
    ...
    /// Initialize a new TableMgr
    /// \param headers Stringlist of the tables headers
    TableMgr(std::vector<std::string> &column_names);
...
private:
    ...
    /// \param tableMgr The TableMgr-object
    /// \return Output stream with the formatted table string
    friend std::ostream &operator<<(std::ostream &out, const TableMgr &tableMgr);
